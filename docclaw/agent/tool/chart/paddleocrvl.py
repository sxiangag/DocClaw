"""PaddleOCR-VL-backed chart implementation."""
...
from docclaw.agent.tool.table.ppstructure import _bboxes_overlap, _extract_bbox as _extract_any_bbox
from docclaw.agent.utils import Action, PageState, Region, RunState, has_searchable_text


class PaddleOCRVLChartTool(ChartTool):
    """Parse charts with PaddleOCR-VL stage-II prompts."""
    ...
    def description(self) -> str:
        return (
            ...
            "with PaddleOCR-VL using the chart prompt. This uses DocClaw for layout "
            "targeting and PaddleOCR-VL for region-level chart recognition."
        )
    ...
    def parse_charts(
        ...
        for page in pages:
            ...
            for candidate in candidates:
                chart = self._parse_candidate(page, candidate)
    ...
    def _parse_candidate(
        ...
        raw = self._recognize_chart(page, candidate)
        blocks = _extract_chart_blocks(raw["raw"])
        if not blocks:
            return None
        primary = blocks[0]
        content = _extract_block_content(primary)
        if not content:
            return None
    ...
    def _recognize_chart(self, page: PageState, candidate: _Candidate) -> dict[str, Any]:
...
def _cached_charts_for_page(
    ...
    *,
    ...
    if not selected_regions:
        return None
    if not all(region.type == "chart" and has_searchable_text(region.text) for region in selected_regions):
        return None
    return [_region_to_chart(region) for region in selected_regions]

def _region_to_chart(region: Region) -> dict[str, Any]:
    return {
...
def _chart_candidates_for_page(
    state: RunState,
    page: PageState,
    *,
...
def _extract_chart_blocks(raw: dict[str, Any]) -> list[dict[str, Any]]:
...
def _extract_block_content(item: dict[str, Any]) -> str | None:
