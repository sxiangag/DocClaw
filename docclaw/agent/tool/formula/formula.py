...
class FormulaTool(Tool):
    """Base class for tools that expose formula structure from document pages."""
    ...
    def description(self) -> str:
        return (
            "Parse formulas from a page set or a known formula-candidate region set "
            "and return structured formula content. Parsed formula results are "
            "written back into page state, and repeating formula parsing on the same "
            "targets usually has no value. Use page_ids exposed in document memory "
            "for page-level parsing, and use known formula-like region_ids from "
    ...
    def target_schema(self) -> dict[str, Any]:
        return {
            "type": "object",
            "description": (
                "Select exactly one target mode. Use page_ids to parse formulas "
                "from whole pages. Use region_ids only for specific known formula-candidate "
                "regions. If you want all formulas from selected pages, use page_ids "
            ...
            "properties": {
                "mode": {
                    "type": "string",
                    "description": (
                        "Explicit formula target mode. Use 'page' for page_ids, "
                ...
                "page_ids": {
                    "type": "array",
                    "description": (
                        "Specific pages to inspect for whole-page formula parsing. "
                ...
                "region_ids": {
                    "type": "array",
                    "description": (
                        "Known region identifiers to treat as explicit formula "
                ...
                "zoom_region_ids": {
                    ...
                    "description": (
                        ...
                        "as explicit formula candidates."
                    ),
                    "items": {"type": "string"},
                    "minItems": 1,
                },
    ...
    def parameters_schema(self) -> dict[str, Any]:
        return {
            "type": "object",
            "description": "Formula parsing controls.",
            "properties": {},
            "additionalProperties": False,
        }
    ...
    async def execute(self, state: RunState, action: Action) -> Observation:
        ...
        try:
            results = self.parse_formulas(state, pages, regions, action)
    ...
    def parse_formulas(
        ...
        """Return normalized formula results for the selected pages or region target."""
...
def _resolve_formula_target(
    ...
    if "page_index" in action.target or "region_id" in action.target:
        return None, None, "parse_formula requires target.page_ids, target.region_ids, target.zoom_region_ids, or target.crop_region_ids"

    mode, error = _optional_target_mode(action.target.get("mode"), action_type="parse_formula")
    if error is not None:
    ...
    if mode_count > 1:
        return None, None, (
            "parse_formula target must specify exactly one of "
