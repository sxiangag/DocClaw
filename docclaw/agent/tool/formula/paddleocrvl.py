"""PaddleOCR-VL-backed formula implementation."""
...
class PaddleOCRVLFormulaTool(FormulaTool):
    """Parse formulas with PaddleOCR-VL stage-II prompts."""

    DEFAULT_MODEL_NAME = "PaddleOCR-VL"
    ...
    def description(self) -> str:
        return (
            ...
            "with PaddleOCR-VL using the formula prompt. This uses DocClaw for layout "
            "targeting and PaddleOCR-VL for region-level formula recognition."
        )
    ...
    def parse_formulas(
        ...
        for page in pages:
            ...
            for candidate in candidates:
                formula = self._parse_candidate(page, candidate)
    ...
    def _parse_candidate(
        ...
        raw = self._recognize_formula(page, candidate)
        blocks = _extract_formula_blocks(raw["raw"])
        if not blocks:
            return None
        primary = blocks[0]
        latex = _extract_block_content(primary)
        if not latex:
            return None
    ...
    def _recognize_formula(self, page: PageState, candidate: _Candidate) -> dict[str, Any]:
...
def _formula_candidates_for_page(
    state: RunState,
    page: PageState,
    *,
...
def _extract_formula_blocks(raw: dict[str, Any]) -> list[dict[str, Any]]:
