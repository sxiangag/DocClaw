...
class TableTool(Tool):
    """Base class for tools that expose table structure from document pages."""
    ...
    def description(self) -> str:
        return (
            "Parse tables from a page set or a known table-candidate region set and "
            "return structured table content. Parsed table results are "
            "written back into page state, and repeating table parsing on the same "
            "targets usually has no value. Use page_ids exposed in document memory "
            "for page-level parsing, and use known table-like region_ids from "
    ...
    def target_schema(self) -> dict[str, Any]:
        return {
            "type": "object",
            "description": (
                "Select exactly one target mode. Use page_ids to parse tables "
                "from whole pages. Use region_ids only for specific known table-candidate "
                "regions. If you want all tables from selected pages, use page_ids "
            ...
            "properties": {
                "mode": {
                    "type": "string",
                    "description": (
                        "Explicit table target mode. Use 'page' for page_ids, "
                ...
                "page_ids": {
                    "type": "array",
                    "description": "Specific page ids to inspect for whole-page table parsing. When using page_ids, region_ids should be [] or omitted.",
                    "items": {"type": "string"},
                    "minItems": 1,
                },
                "region_ids": {
                    "type": "array",
                    "description": "Known region identifiers to treat as explicit table candidates. When using page_ids, set this to [] or omit it.",
                    "items": {"type": "string"},
                    "minItems": 1,
                },
                "zoom_region_ids": {
                    "type": "array",
                    "description": "Known region identifiers whose zoom artifacts should be treated as explicit table candidates.",
                    "items": {"type": "string"},
                    "minItems": 1,
                },
                "crop_region_ids": {
                    "type": "array",
                    "description": "Known region identifiers whose crop artifacts should be treated as explicit table candidates.",
                    "items": {"type": "string"},
                    "minItems": 1,
                },
            },
    ...
    def parameters_schema(self) -> dict[str, Any]:
        return {
            "type": "object",
            "description": "Table parsing controls.",
            "properties": {},
            "additionalProperties": False,
        }
    ...
    async def execute(self, state: RunState, action: Action) -> Observation:
        ...
        try:
            results = self.parse_tables(state, pages, regions, action)
    ...
    def parse_tables(
        ...
        """Return normalized table results for the selected pages or region target."""
...
def _resolve_table_target(
    ...
    if "page_index" in action.target or "region_id" in action.target:
        return None, None, "parse_table requires target.page_ids, target.region_ids, target.zoom_region_ids, or target.crop_region_ids"

    mode, error = _optional_target_mode(action.target.get("mode"), action_type="parse_table")
    if error is not None:
    ...
    if mode_count > 1:
        return None, None, "parse_table target must specify exactly one of target.page_ids, target.region_ids, target.zoom_region_ids, or target.crop_region_ids"
